...
//        print("📏 Расстояние до последней точки: \(distance) м")
                ...
                if let httpResponse = response as? HTTPURLResponse {
                    if httpResponse.statusCode == 200 {
                        ...
                        if let data = data, let errorMessage = String(data: data, encoding: .utf8) {
                            if errorMessage.contains("JWT token is expired") || errorMessage.contains("JWT token is invalid") {
                                print("⚠️ Токен устарел, обновляем...")
                                self?.refreshJWTToken { success in
                                    if success {
                                        self?.sendLocationToServer(coordinate, retry: false)
                                    } else {
                                        print("⛔ Ошибка обновления токена location")
                                    }
                                }
                            } else {
                                print("❌ Серверная ошибка: \(errorMessage)")
                            }
                        } else {
                            print("❌ Ошибка 500 без тела ответа")
                        }
                    } else {
    ...
    private func refreshJWTToken(completion: @escaping (Bool) -> Void) {
        guard let url = URL(string: "\(Constants.baseURL)/api/auth/refresh") else {
            print("❌ Ошибка: Неверный URL для обновления токена")
            completion(false)
            return
        }
        
        let storedRefreshToken = "eyJhbGciOiJIUzM4NCJ9.eyJ1c2VySWQiOiJmMTIzNDM0MS1kMjRhLTQyOGEtOGQ1MS04ZWUyZTE4M2U4MTQiLCJpYXQiOjE3NDU4MjE5MjcsImV4cCI6MTc0ODQ1MTY3MX0.0jKJmhk3iYWe6pRLiiaKgdMAGVtVYWAB4qJ-tuHGv0WtEE1fI5ig5xJqbjkyr3mP"
        

        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        request.setValue("Bearer \(storedRefreshToken)", forHTTPHeaderField: "Authorization")
        
        URLSession.shared.dataTask(with: request) { data, response, error in
            DispatchQueue.main.async {
                if let error = error {
                    print("❌ Ошибка запроса при обновлении токена: \(error.localizedDescription)")
                    completion(false)
                    return
                }
                
                guard let httpResponse = response as? HTTPURLResponse else {
                    print("❌ Ошибка: Нет HTTP-ответа при обновлении токена")
                    completion(false)
                    return
                }
                ...
                guard httpResponse.statusCode == 200 else {
                    if let data = data, let serverResponse = String(data: data, encoding: .utf8) {
                        print("❌ Статус: \(httpResponse.statusCode), ответ: \(serverResponse)")
                    } else {
                ...
                guard let data = data, let newAccessToken = String(data: data, encoding: .utf8) else {
                    print("❌ Ошибка: Не удалось прочитать новый токен")
                    completion(false)
                    return
                }
                
                UserDefaults.standard.set(newAccessToken, forKey: "jwtToken")
                print("✅ Токен успешно обновлен: \(newAccessToken)")
                completion(true)
            }
        }.resume()
    }
