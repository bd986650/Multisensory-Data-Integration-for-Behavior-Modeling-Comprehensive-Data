                ...
                if let httpResponse = response as? HTTPURLResponse {
                    if httpResponse.statusCode == 200 {
                        ...
                        if let data = data, let errorMessage = String(data: data, encoding: .utf8) {
                            print("Ошибка 500. Тело ответа: \(errorMessage)")
                            if errorMessage.contains("JWT token is expired") {
//                                print("⚠️ Токен устарел, обновляем...")
//                                self?.refreshJWTToken { success in
//                                    if success {
//                                        self?.sendStepsToServer(steps: steps)
//                                    } else {
...
//                                }
                            ...
                            if errorMessage.contains("JWT token is expired") || errorMessage.contains("JWT token is invalid") {
                                ...
                                self?.refreshJWTToken { success in
                                    if success {
                                        ...
                                        print("⛔ Ошибка обновления токена health")
                                    }
                                }
                            }
                        }
    ...
    private func refreshJWTToken(completion: @escaping (Bool) -> Void) {
        ...
        guard let storedRefreshToken = UserDefaults.standard.string(forKey: "refreshToken") else {
            print("❌ Ошибка: Нет refreshToken в UserDefaults")
            completion(false)
            return
        }
            ...
            DispatchQueue.main.async {
                ...
                guard httpResponse.statusCode == 200 else {
                    if let data = data, let serverResponse = String(data: data, encoding: .utf8) {
                        print("❌ Ошибка обновления токена. Статус: \(httpResponse.statusCode), ответ: \(serverResponse)")
                    } else {
                        print("❌ Ошибка обновления токена. Статус: \(httpResponse.statusCode)")
                    }
                    completion(false)
                    return
                }
